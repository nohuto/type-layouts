struct _POOL_TRACKER_BIG_PAGES// Size=0x20 (Id=2155)
{
    unsigned long long Va;// Offset=0x0 Size=0x8
    unsigned long Key;// Offset=0x8 Size=0x4
    struct // Size=0x4 (Id=0)
    {
        unsigned long Pattern:8;// Offset=0xc Size=0x4 BitOffset=0x0 BitSize=0x8
        unsigned long PoolFlags:12;// Offset=0xc Size=0x4 BitOffset=0x8 BitSize=0xc
        unsigned long SlushSize:12;// Offset=0xc Size=0x4 BitOffset=0x14 BitSize=0xc
    };
    unsigned long long NumberOfBytes;// Offset=0x10 Size=0x8
    struct _EPROCESS * ProcessBilled;// Offset=0x18 Size=0x8
};
