enum _WdfUsbTargetDeviceSelectSettingType
{
    WdfUsbInterfaceSelectSettingTypeDescriptor=16,
    WdfUsbInterfaceSelectSettingTypeSetting=17,
    WdfUsbInterfaceSelectSettingTypeUrb=18
};

struct _unnamed_368// Size=0x8 (Id=368)
{
    struct _USB_INTERFACE_DESCRIPTOR * InterfaceDescriptor;// Offset=0x0 Size=0x8
};

struct _unnamed_369// Size=0x1 (Id=369)
{
    unsigned char SettingIndex;// Offset=0x0 Size=0x1
};

struct _unnamed_359// Size=0x8 (Id=359)
{
    struct _URB * Urb;// Offset=0x0 Size=0x8
};

union _unnamed_370// Size=0x8 (Id=370)
{
    struct _unnamed_368 Descriptor;// Offset=0x0 Size=0x8
    struct _unnamed_369 Interface;// Offset=0x0 Size=0x1
    struct _unnamed_359 Urb;// Offset=0x0 Size=0x8
};

struct _WDF_USB_INTERFACE_SELECT_SETTING_PARAMS// Size=0x10 (Id=371)
{
    unsigned long Size;// Offset=0x0 Size=0x4
    enum _WdfUsbTargetDeviceSelectSettingType Type;// Offset=0x4 Size=0x4
    union _unnamed_370 Types;// Offset=0x8 Size=0x8
};
