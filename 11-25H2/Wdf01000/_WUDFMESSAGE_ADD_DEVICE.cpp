enum _WUDFMESSAGE
{
    ...
    WudfMsg_Custom_Event=36,
    WudfMsg_Max=37
};

struct _WUDFMESSAGE_HEADER// Size=0x1 (Id=1172)
{
    enum _WUDFMESSAGE Message;// Offset=0x0 Size=0x1
};

struct _WUDFMESSAGE_ADD_DEVICE// Size=0x28 (Id=1626)
{
    struct _WUDFMESSAGE_HEADER Header;// Offset=0x0 Size=0x1
    unsigned long AttachDeviceNameOffset;// Offset=0x4 Size=0x4
    unsigned char InternalReferenceStringValid[2];// Offset=0x8 Size=0x2
    unsigned long InternalReferenceStringOffset[2];// Offset=0xc Size=0x8
    unsigned long PdoNameOffset;// Offset=0x14 Size=0x4
    void * DeviceObjectPointer;// Offset=0x18 Size=0x8
    long Validate(void * );
    struct WUDF_DEVNODE_CONTEXT__ * DevnodeContext;// Offset=0x20 Size=0x8
};
